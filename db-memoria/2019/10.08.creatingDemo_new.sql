...
600 - 3-letter notation language, name, description, icon hash. It means that the project is a demo.
';

-- 3 буквенный код берется из таблицы lang

/*
Запросы для добавления/редактирования и удаления проекта из дэмона

"attributeProject"
"attributeProjectSet"
если в attributeProjectSet передать в качестве значения null, атрибут удалится

Я там не смогу проверять на то что проект относится к демо вселенной, но буду проверять в запросе на проекты

атрибут:
600, "json" -  язык проекта, имя, дескрипшен и хеш иконки проекта в json, хотя можно и в каком либо другом формате, не принципиально, главное чтобы удобно было писать и читать. 
Если этот ключ задан, то проект в дэмоне

По поводу хеша, все-таки решил что проще всем будет если заливать иконку в каргадор. адрес каргадора storage.cerebrohq.com, папка Cerebro-icons, потом Дима её от туда загрузит. Формат я думаю svg.
*/
...
begin
	...
	--create groups and perms
	--zCreatePromoUniverse_00
	--perm_roles	
																 
	-- Add admins (_u_admins and _me)	
	
	IF array_length(_u_admins, 1) > 0
	THEN
		SELECT uid INTO _id FROM groups WHERE name = _name_uni || ' admins' and unid = _unid_to;
		IF _id is null
		THEN
			INSERT INTO groups(name, unid) VALUES (_name_uni || ' admins', _unid_to);
			SELECT uid INTO _id FROM groups WHERE name = _name_uni || ' admins' and unid = _unid_to;	
			INSERT INTO perm_groups(groupid, taskid, privileg) VALUES (_id, 0, -2);
		END IF;
	...
	THEN
		SELECT uid INTO _id FROM groups WHERE name = _name_uni || ' workers' and unid = _unid_to;
		IF _id is null
		THEN
			INSERT INTO groups(name, unid) VALUES (_name_uni || ' workers', _unid_to);
			SELECT uid INTO _id FROM groups WHERE name = _name_uni || ' workers' and unid = _unid_to;	
			INSERT INTO perm_groups(groupid, taskid, privileg) VALUES (_id, 0, 8607801348);			   
		END IF;															 
