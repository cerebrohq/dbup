...
Изменения в копировании задач
Добавлен новый флаг 4096 для копирования статуса и прогреса.

Изменено копирование сообщений. Сообщения теперь копируются без сохранения связей, но с сохранением порядка, иначе там каша получается.
разница между dupVTask и dupVTask_CrossUni, в том, что в последней не копируются сообщения об изменении статуса

Изменено:

Процедуры копирования задач:
dupVTask
dupVTask_CrossUni

*/

-- FUNCTION: "dupVTask"(bigint[], text[], bigint, integer, integer[])

-- DROP FUNCTION "dupVTask"(bigint[], text[], bigint, integer, integer[]);

CREATE OR REPLACE FUNCTION "dupVTask"(
	__tid bigint[],
	__name text[],
	_pid bigint,
	_flags integer,
...
*/
	...
	CREATE INDEX ix_tt_dup_tasks_dst ON tt_dup_tasks(dst);

	CREATE TEMP TABLE tt_dup_refs
	(
	   num int NOT NULL,
	   src bigint NOT NULL,
	   dst bigint NOT NULL,
	...
	CREATE INDEX ix_tt_tt_dup_refs_src ON tt_dup_refs(src);
	CREATE INDEX ix_tt_tt_dup_refs_dst ON tt_dup_refs(dst);
	...
	loop
		...
		then
			-- reference
			insert into tt_dup_refs(src, dst, dst_parent, new_name, num)
			...
			if (_copy_prj)
			then
				insert into tt_dup_tasks(src, dst, dst_parent, new_name, deep, url, perms, num, langid, fts)
	...
	_dup_mode = (exists (select 1 from tt_dup_tasks group by src having count(src)>1));
	...
	then
		...
		loop
			...
			insert into tt_dup_refs(src, dst, dst_parent, num) 
				select nav_links.uid, "newID"(), tt.dst, num
					from nav_links join tt_dup_tasks as tt on nav_links.parentid=tt.src
					where tt.deep=i -- and (tasks.flags & 1)=0 and ("perm"(_usid, tasks.uid, 0) & _perm_mask) = _perm_mask
					order by seq_order;
	...
	select 
		...
		from nav_links join tt_dup_refs as tt on nav_links.uid = tt.src
		order by tt.dst;
	...
	then
		...
		insert into tag_val_enum(taskid, tagid, enumid)
				...
				from tag_val_enum join tt_dup_tasks as tt on tag_val_enum.taskid = tt.src
				where del=0;

		insert into tag_val_scalar(taskid, tagid, ival, rval, sval)
				select     tt.dst, tagid, ival, rval, sval
				   from tag_val_scalar join tt_dup_tasks as tt on tag_val_scalar.taskid = tt.src
				   where del=0;
	end if;
	...
	then
		...
		then
			perform "userAssignmentSet"(userid, tt.dst, true)
					from users_tasks join tt_dup_tasks as tt on users_tasks.taskid = tt.src
					where 
	...
	if((_flags & 64)!=0)
	then
		--CREATE INDEX ix_tt_dup_tasks_num ON tt_dup_tasks(num);
		...
		then
			...
			loop
				insert into links(src,    dst, flags, percent)
					...
					from links  
						join tt_dup_tasks as ta on links.src=ta.src 
						join tt_dup_tasks as tb on links.dst=tb.src 
	...
	then
		insert into links(src,       dst, flags, percent)
			...
			from links
				join tt_dup_tasks as ta on links.src=ta.src 
			where del=0 and not exists (select 1 from tt_dup_tasks where src=links.dst);
	end if;
	...
	then
		insert into links(  src,     dst, flags, percent)
			...
			from links 
				join tt_dup_tasks as tb on links.dst=tb.src 
			where del=0 and not exists (select 1 from tt_dup_tasks where src=links.src);
	end if;
	...
	then
		insert into perm_groups(mtm,  muid, groupid, taskid, privileg, flags)
				...
				from perm_groups join tt_dup_tasks as tt on taskid = tt.src;

		insert into perm_users( mtm,  muid, userid, taskid, privileg, flags)
				select _now, _usid, userid, tt.dst, privileg, flags
				from perm_users join tt_dup_tasks as tt on taskid = tt.src;
	end if;
	...
	return query 
			(select dst from tt_dup_tasks) union (select dst from tt_dup_refs);
		--select count(1) from tt_dup_tasks group by deep order by deep; --limit 10;
END

$BODY$;

ALTER FUNCTION "dupVTask"(bigint[], text[], bigint, integer, integer[])
    OWNER TO sa;


-- FUNCTION: "dupVTask_CrossUni"(bigint[], text[], bigint, integer, integer[])

-- DROP FUNCTION "dupVTask_CrossUni"(bigint[], text[], bigint, integer, integer[]);

CREATE OR REPLACE FUNCTION "dupVTask_CrossUni"(
	tid bigint[],
	_name text[],
	_pid bigint,
	_flags integer,
...
--		end if;
	...
	return query 
		select dst from tt_dup_tasks;
		--select count(1) from tt_dup_tasks group by deep order by deep; --limit 10;
END

$BODY$;

ALTER FUNCTION "dupVTask_CrossUni"(bigint[], text[], bigint, integer, integer[])
    OWNER TO sa;
